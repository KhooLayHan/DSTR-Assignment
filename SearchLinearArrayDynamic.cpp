...
namespace PerformanceEvaluation
{
    ...
    DynamicArray<Dataset> SearchLinearDynamicArray::LinearSearchAndCopy(std::string_view target, const DynamicArray<Dataset>& array, Criteria criteria, SearchType type) {
        ...
        for (size_t i = 0; i < array.GetLength(); ++i) {
            ...
            auto is_found = [&](std::string_view target, std::string_view dataset_str) -> bool {
                ...
                //     new_array.Insert(new_array.GetLength(), dataset); // Copy dataset
                // };
            };
    ...
    constexpr bool SearchLinearDynamicArray::Contains(std::string_view target, std::string_view dataset_str, SearchType type) {
        switch (type) {
    ...
    constexpr std::string SearchLinearDynamicArray::ToLowerCase(std::string_view dataset) {
        std::string result;
